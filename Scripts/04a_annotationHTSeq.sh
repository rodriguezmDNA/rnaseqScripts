...
display_help() {
    ...
    echo ""
    echo -e "\n Assumptions:"
    echo "* A directory '03_alignment/{bwt1,bwt2}_genome/' with genome annotated reads in bam format "
...
while getopts ':hv:a:p:' option; do
  case "$option" in
    h) display_help
       exit
       ;;
...
## Read
inputFolder=$optionBowtie"_genome"
...
## Count stats:
logCount=$DIR/logs/Counts_$inputFolder #Create log folder for the counts.
